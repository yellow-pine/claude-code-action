...
⚠️ **Security Warning**: Never use `trusted_actors` with `pull_request_target` events, as this runs with elevated permissions and could allow malicious code execution. The action will always require full permission checks for `pull_request_target` events regardless of `trusted_actors` configuration.
