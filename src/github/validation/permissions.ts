...
export async function checkWritePermissions(
  ...
  // Parse allowed bots list for enhanced checking
  const allowedBotsList = inputs.allowedBots
    ? inputs.allowedBots
        .split(",")
        .map((bot) => bot.trim())
  ...
  if (allowedBotsList.length) {
    core.info(`Allowed bots configured: ${allowedBotsList.join(", ")}`);
  }

  // Check if the actor is a GitHub App (bot user) that should skip permission checks
  if (actor.endsWith("[bot]")) {
    core.info(`Actor is a GitHub App: ${actor}`);
    // Only skip check if bot is in allowed list or all bots are allowed
    if (
      inputs.allowedBots === "*" ||
      allowedBotsList.some(
        (bot) =>
          actor.toLowerCase().replace(/\[bot\]$/, "") ===
          bot.toLowerCase().replace(/\[bot\]$/, ""),
      )
    ) {
      core.info(`Bot ${actor} is allowed - using enhanced token verification`);
      // Continue to enhanced token verification below
    } else {
      core.info(
        `Bot ${actor} is not in allowed list - continuing with full permission check`,
      );
    }
  }
